...
# Compute f1 score
def ComputeF1(y_real, y_pred):

    score = f1_score(y_real, y_pred.round())
...
def SelectBest(history_all, file_path, fold, monitor='val_loss'):
    if monitor == 'val_loss':
       ...
       accuracy = 0.
       for num, History in history_all.items():
           if np.max(History.history['val_accuracy']) > accuracy:
              best_num = int(num)
