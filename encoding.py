...
def embed_kmer(seq,mapper,instance_len,instance_stride,kernelsize,reverse,kmer):
    ...
    for i in range(instance_num):
        ...
        if len(instance_fw) < instance_len:
           #实例长度不一致
           print("the length of instance is not consistent.", file=sys.stderr); sys.exit(1)
    ...
    if run == 'ws':
       ...
       if kmer == 1:
          ...
          for seq in seqs:
              ...
              mat = [ mapper[element] for element in fw_kmer if element in mapper]
              seqs_vector.append(mat)
...
def parse_args():
    ...
    parser.add_argument("posfile",  type=str, help="Positive sequences in FASTA/TSV format (with .fa/.fasta or .tsv extension)", default=r'D:\workship\anewlife\wscnnlstmchange\rawdata\Gm12878\positive.txt')
    parser.add_argument("negfile",  type=str,help="Negative sequences in FASTA/TSV format", default=r'D:\workship\anewlife\wscnnlstmchange\rawdata\Gm12878\negative.txt')
    parser.add_argument("outfile",  type=str, help="Output file (example: $MODEL_TOPDIR$/data/train.hdf5). ", default=r'D:\workship\anewlife\wscnnlstmchange\rawdata\Gm12878\datalabel.hdf5')
    ...
    parser.add_argument("-c", "--instance_len", dest="instance_len", type=int, default=50, help="The length of instance")
    parser.add_argument("-s", "--instance_stride", dest="instance_stride", type=int, default=10, help="The stride of getting instance")
