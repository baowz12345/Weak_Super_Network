...
from sklearn.metrics import accuracy_score, roc_curve, auc, precision_recall_curve, f1_score, matthews_corrcoef
from sklearn.model_selection import KFold
from my_utils1 import AllSample, SelectBest, PlotandSave, Id_k_folds
from my_models1 import RNNls, WSCNNLSTMwithNoisy, DeepBind, DanQ, DeepSite
...
def parse_args():
    ...
    parser.add_argument('-datalable', dest='datalable', type=str, help='Data for training, testing', default=r'D:\workship\anewlife\wscnnlstmchange\rawdata\Gm12878\data_nows1\datalabel.hdf5')
    # parser.add_argument('-datalable', dest='datalable', type=str, help='Data for training, testing',default=r'D:\workship\anewlife\wscnnlstmchange\rawdata\wgEncodeHaibTfbsGm12878BatfPcr1xPkRep1\data_ws1\datalabel.hdf5')
    ...
    parser.add_argument('-params', dest='params', type=int,default=12, help='the number of paramter settings')
...
def main():
    ...
    if not exists(r'D:\workship\anewlife\wscnnlstmchange\model\my_test1'):
        print('Building D:\workship\ anewlife\wscnnlstmchange\model\my_test1')
        os.makedirs(r'D:\workship\anewlife\wscnnlstmchange\model\my_test1')
    f_params = open(r'D:\workship\anewlife\wscnnlstmchange\model\my_test1\params.txt', 'w')
    ...
    for fold in range(args.k_folds):
        ...
        if args.train:
            ...
            for params_num in range(args.params):
                ...
                if args.run == 'nows':
                    model = DeepSite(input_shape, params)
                else:
                    model = WSCNNLSTMwithNoisy(input_shape, params)
                checkpointer = ModelCheckpoint(filepath = r'D:\workship\anewlife\wscnnlstmchange\model\my_test1\params%d_bestmodel_%dfold.hdf5' %( params_num, fold), monitor='val_accuracy', verbose=1, save_best_only=True)
                earlystopper = EarlyStopping(monitor='val_accuracy', patience=10, verbose=1)

                print('Training model...')
                myoptimizer = SGD(decay=params['DELTA'], momentum=params['MOMENT'],learning_rate =0.001)
                ...
                History = model.fit(X_train, y_train, epochs=100, batch_size=args.batchsize, shuffle=True, validation_data=(X_valid, y_valid), callbacks=[checkpointer, earlystopper], verbose=2)
                history_all[str(params_num)] = History

            best_num = SelectBest(history_all, file_path + '/model/%s/' % name, fold, 'val_accuracy')
            if args.plot:
                PlotandSave(history_all[str(best_num)], r'D:\workship\anewlife\wscnnlstmchange\model\my_test1\figure_%dfold.png' % (fold), fold, 'val_accuracy')

        print('Begin testing model...')
        model.load_weights(r'D:\workship\anewlife\wscnnlstmchange\model\my_test1\params%d_bestmodel_%dfold.hdf5' % ( best_num, fold))
        ...
        #将结果写入文件
        with open(r'D:\workship\anewlife\wscnnlstmchange\model\my_test1\score_%dfold.txt' %(fold), 'w') as f:
        ...
        #f1score
        f1score = f1_score(y_real,y_pred.round())
        f1score1.append(f1score)

        #MCC
        MCC = matthews_corrcoef(y_real,y_pred.round())
        MCC1.append(MCC)
        #添加评价指标
        preci1.append(np.mean(precision))
        reca1.append(np.mean(recall))
    ...
    print("the average accuracy is {}".format(np.mean(acc1)))
    print("the average auc is {}".format(np.mean(roc_auc1)))
    print("the average f1score is {}".format(np.mean(f1score1)))
    print("the average MCC is {}".format((np.mean(MCC1))))
    print("the average precision is {}".format(np.mean(preci1)))
    print("the average recall is {}".format(np.mean(reca1)))




    outfile = r'D:\workship\anewlife\wscnnlstmchange\model\my_test1\metrics.txt'
    with open(outfile,'w') as f:
        for i in range(len(roc_auc1)):
            print("accuracy:{:.5f} roc_auc:{:.5f} f1score is {:.5f} MCC:{:.5f} precision:{:.5f} recall:{:.5f} ".format(acc1[i], roc_auc1[i], f1score1[i], MCC1[i], preci1[i],reca1[i] ), file=f)
            #print("precision is {:.5f} recall is {:.5f} ".format(preci1[i][0], reca1[i][0]), file=f)

        print("mean accuracy is {:.5f} mean roc_auc:{:.5f} mean f1score:{:.5f} mean precision:{:.5f} mean recall:{:.5f}".format(np.mean(acc1),np.mean(roc_auc1), np.mean(f1score1), np.mean(MCC1), np.mean(preci1), np.mean(reca1)), file=f)
        #print("mean precision is {:.5f} mean precision is {:.5f}".format(np.mean(preci1), np.mean(reca1)), file=f)
